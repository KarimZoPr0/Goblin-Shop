...
public class ObjectSettings : MonoBehaviour
{
    ...
    public Vector3 HomePos;//MY EDIT
    Vector3 MousePos;
    [HideInInspector]
    ...
    float differenceY;
    //
    [Header("Get reference of the item data")]
    public GSS.Inventory.GenericItem itemData;
    public PanelSettings curPanel;
    // Customization Tools
    ...
    {
        GetGameObjects();
        Vector3 getPos = homeTransform.position;
        HomePos = GetComponent<RectTransform>().anchoredPosition; //thisRT.position;
    }
    ...
    void CheckObjectPos()
    {
        ...
        { // check if drag & drop failed
            ...
            {
				StartCoroutine(SmoothMove("User", thisRT,fncReturnPos(), ReturnSpeed));///replace currentPos with fncReturnPos
    ...
    Vector3 fncReturnPos()
    {
		
        bool isChkout = false;
        string getID = DragDropManager.GetObjectPanel(Id);
        //
        for (int i = 0; i < DDM.AllPanels.Length; i++)
        {
            if (DDM.AllPanels[i].id == getID)//&&DDM.AllPanels[i].isCheckout){
                isChkout = DDM.AllPanels[i].isCheckout;
            if (isChkout)
            {
                print($"//PANELCHECK: panel {DDM.AllPanels[i].id} is on the checkout counter");
                break;
            }
        }
        //
        return isChkout?HomePos:CurrentPos;
    }
    ...
    void DropActions(int i)
    {
        ...
        DDM.AllPanels[i].SetupPanelEvents(itemData);
            ...
            if (DDM.IdManager[j] == Id)
            {
                ...
                for (int k = 0; k < DDM.AllObjects.Length; k++)
                {
                    if (DDM.IdManager[i] == DDM.AllObjects[k].Id)
                    {
                        ...
                        {
                            StartCoroutine(SmoothMove("User", DDM.AllObjects[k].GetComponent<RectTransform>(), fncReturnPos(), MovementSpeed));
                        }
                    }
                }
                return true;
