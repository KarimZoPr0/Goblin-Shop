...
public class DragDropManager : MonoBehaviour {
    ...
    public ObjectSettings curObj{ get; private set; }
    public void fncSetDragObj(ObjectSettings thisObj)
    {
        if (thisObj != null)
        {
            curObj = thisObj;
            print($"//DDM: Current object being dragged is : {thisObj.Id}");
        }
    }
    ...
    // public void fncSelectItem(ObjectSettings thisItem)
    // {
    //     bool toCounter = false;
    //     for (int i = 0; i < AllPanels.Length; i++)
    //     {
    //         if (!AllPanels[i].isCheckout) continue;
    //         string getID = GetPanelObject(AllPanels[i].id);
    //         if (!string.IsNullOrEmpty(getID)) continue;
    //         else
    //         {
    //             StartCoroutine(fncMoveToCounter(thisItem));
    //         }
    //     }
    // }
	// System.Collections.IEnumerator fncMoveToCounter(ObjectSettings thisItem){
    //     thisItem.AIControl = true;
    //     yield return null;
